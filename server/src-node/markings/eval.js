...
function evalExcessiveMarkingsExclamationQuestionMarks(metrics) {
	// two consecutive question marks or exclamation marks => 0.3
	// more than two consecutive => 0.6
	...
	/*let excessive = null
	let rating
	if (/!!!/.test(metrics.textClean)) {
		rating = 0.6
		excessive = '!!!'
	}
	else if (/\?\?\?/.test(metrics.textClean)) {
		rating = 0.6
		excessive = '???'
	}
	else if (/!!/.test(metrics.textClean)) {
